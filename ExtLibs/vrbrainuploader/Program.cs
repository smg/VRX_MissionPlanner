...
namespace vrbrainuploader
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine(@"Usage: vrbrainuploader.exe C:\vrbrain\Firmware\Images\vrbrainv4.vrbrain");

                OpenFileDialog ofd = new OpenFileDialog();
                ofd.Filter = "Firmware (*.vrbrain)|*.vrbrain";
                ofd.FileName = "vrbrainv4.vrbrain";
                if (Directory.Exists(@"C:\vrbrain\Firmware\Images"))
                    ofd.InitialDirectory = @"C:\vrbrain\Firmware\Images";
                ofd.ShowDialog();
